...
function FeatureIcon({ supported }: { supported: boolean }) {
  return supported ? (
    ...
    <span className="inline-flex h-6 w-6 items-center justify-center rounded-full border border-muted">
...
export function UserGuideDialogContent() {
  return (
    <div className="prose prose-sm max-w-none prose-headings:mt-4 prose-headings:mb-2 prose-p:my-2 prose-ul:my-2 prose-li:my-0.5 prose-table:my-2">
      <h1 className="mb-4 border-b pb-2 text-xl font-bold text-foreground">
        秀儿Xiuer直播助手 - 用户操作手册
      </h1>
      ...
      <ReactMarkdown
        components={{
          h1: ({ children }) => (
            <h1 className="mb-4 border-b pb-2 text-xl font-bold text-foreground">{children}</h1>
          ),
          h2: ({ children }) => (
            <h2 className="mt-6 mb-3 flex items-center gap-2 text-lg font-semibold text-foreground">
              {children}
          ...
          ul: ({ children }) => (
            <ul className="ml-4 list-disc space-y-1 text-sm text-muted-foreground">{children}</ul>
          ),
          ol: ({ children }) => (
          ...
          table: ({ children }) => (
            <div className="my-4 overflow-x-auto">
              <table className="w-full overflow-hidden rounded-lg border border-border border-collapse text-sm">
                {children}
              </table>
            </div>
          ),
