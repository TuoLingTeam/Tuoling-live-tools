...
> **最后更新**: 2026-05-26
> **版本主题**: v2.0.0 聚焦多账号直播控制稳定性、自动回复工作台体验、浏览器资源占用控制与后台审计性能
> **历史版本**: 
> - v2.0.0: 发布准备中
> - v1.6.8: 上一稳定版本
> - v1.6.7: 历史稳定版本
...
- **v1.6.8**: "浏览器路径与管理端性能加固版" - 加强浏览器路径校验、自动弹窗扫描稳定性和 auth-api 管理端性能
- **v2.0.0**: "多账号直播控制与自动回复工作台增强版" - 加固直播控制状态、降低浏览器资源占用并增强自动回复工作台

#### 5. 当前正式版本判断标准
- **原则**: 以"最新通过修复后的正式 tag"为准，发布准备阶段必须完成版本 bump、门禁、tag、构建与验证后再对外生效
- **当前准备目标**: v2.0.0 是本次准备进入门禁和发布链路的目标正式版本
- **对外**: 发布完成后统一以 v2.0.0 作为当前正式对外版本
...
| 版本 | API 基线 | 状态 | 说明 |
|------|----------|------|------|
| v2.0.0 | `https://auth.xiuer.work` | 🚧 发布准备中 | 多账号直播控制与自动回复工作台增强版，加固状态判断、降低浏览器资源占用并增强自动回复设置/历史视图 |
| v1.6.8 | `https://auth.xiuer.work` | 📋 历史记录 | 浏览器路径与管理端性能加固版，加强浏览器路径校验、自动弹窗扫描稳定性和 auth-api 管理端性能 |
| v1.6.7 | `https://auth.xiuer.work` | 📋 历史记录 | 崩溃恢复与任务自动恢复版，加固渲染进程崩溃后的自动重连、任务恢复与断连原因识别 |
